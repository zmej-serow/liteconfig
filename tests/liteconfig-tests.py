...
class TestLiteConfig:
    variants = [liteconfig.Config('test.ini'),
    ...
    def test_write(self):
        ...
        cfg1.write('out.ini')
        cfg2 = liteconfig.Config('out.ini')
    ...
    def test_encodings(self):
        cfg = liteconfig.Config('koi8-r.ini', encoding='koi8_r')
        assert cfg.бНОПНЯ == "Вопрос"
        assert not cfg.Вопрос
        cfg = liteconfig.Config('koi8-r.ini', encoding='cp1251')
        assert cfg.Вопрос == "чПРТПУ"
        assert not cfg.чПРТПУ
        with pytest.raises(UnicodeError):
            _ = liteconfig.Config('koi8-r.ini')
